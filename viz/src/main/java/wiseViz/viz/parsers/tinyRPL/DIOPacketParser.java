...
/**
 * Inspects the trace file for DIO packets.
 */
public class DIOPacketParser
     ...
     */
    public DIOPacketParser(final VizPanel vPanel) {
        super(vPanel);
    }
    ...
    public void update(final Observable obj, final Object arg) {
        ...
        for (VizLink vizLink : thisNode.getLinks()) {
            ...
            if (vizLink.getTarget().getId() == thisNode.getId()) {
                thisNode.sendPacket(vizLink, Color.RED.getRGB(), 4, thisNode, vizLink.getTarget());
            } else {
                thisNode.sendPacket(vizLink, Color.RED.getRGB(), 4, thisNode, vizLink.getSource());
            }
        }
    }
